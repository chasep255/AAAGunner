...
namespace btk::ballistics
{
  ...
  // Litz spin drift is an empirical displacement curve rather than a physical
  // force. Apply its per-step displacement directly so aerodynamic drag does
  // not attenuate a lateral velocity introduced solely to reproduce the fit.
  float Simulator::computeSpinDriftDisplacement(float t) const
  {
    if(sg_ <= 0.0f || t <= 0.0f)
      return 0.0f;

    float drift_inches = 1.25f * (sg_ + 1.2f) * std::pow(t, 1.83f);
    return btk::math::Conversions::inchesToMeters(drift_inches);
  }
  ...
  void Simulator::simulate(float max_distance, float dt, float max_time)
  {
    ...
    {
      // Honor the caller's time budget even when it is not a multiple of dt.
      timeStep(std::min(dt, max_sim_time - current_time_));
  ...
  void Simulator::timeStep(float dt)
  {
    ...
    // Add exactly the increment prescribed by the empirical displacement curve.
    // This correction changes trajectory position only; it is not a force or an
    // air-relative velocity for the drag model to act on.
    float drift_increment = computeSpinDriftDisplacement(t0 + dt) - computeSpinDriftDisplacement(t0);
    x1 += horizontalRight(s0.getVelocity()) * (drift_increment * static_cast<float>(twist_hand_));
